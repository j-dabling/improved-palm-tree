...
%% API
-export([start_link/1,stop/0,next/1]).
...
start_link(Identifiers) ->
    gen_server:start_link({local, rr_selector}, ?MODULE, Identifiers, []).
%%--------------------------------------------------------------------
%% @doc
...
% -spec next() -> term().
next({Operation,Number})-> gen_server:call(?MODULE, {Operation,Number}).
...
-spec handle_call(Message::term(), From::pid(), State::term()) ->
                                  ...
                                  {stop, term(), term()}.
handle_call({Operation,Number}, _From, Identifiers) ->
        [H|T] = Identifiers,
        Rotated = T ++ [H],
        {reply,gen_server:call(H,{Operation,Number}),Rotated};
